        ...
        if reins_type == "INWARD":
            ...
            with st.expander("🧾 Informasi Voucher", expanded=True):
                ...
                with col1:
                    ...
                    inward_vin = st.text_input("Inward Vin Ref")
                    ...
                    try:
                        ...
                        if biz_type in ["Kontribusi", "Refund", "Alteration", "Retur", "Revise", "Batal", "Cancel"]:
                            log_entry = {
                                ...
                                "Overiding": df["retro overriding"].sum() if "retro overriding" in df.columns else 0,
                                ...
                                "Ujrah Spc (IDR)": "",
                                "Claim (IDR)": 0,
                                "Balance (IDR)": (df["retro total premium"].sum() - df["retro total comm"].sum() - (df["retro overriding"].sum() if "retro overriding" in df.columns else 0) - (df["claim"].sum() if "claim" in df.columns else 0))*rate_exchange,
                                "Check Balance (IDR)":"",
                                "REMARKS": remarks,
                                "STATUS": "POSTED",
                                #"ENTRY_TYPE": entry_type,
